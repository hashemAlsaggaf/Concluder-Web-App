...
function clearSelection(){
	$('.node').each(function(){
		$(this).css({'background-color': nodesBackColor, 'color': 'white'}); 
	});
	selectedNode = null;
}
/*
	...
	*** this function is located in this file so tree config can access it
	
*/
var nodeMenuBg = $('#nodeMenuBg');
var nodeMenu = $('#nodeMenu');
var analyzeNodeBtn = $('#analyzeNodeBtn');
...
var defaultNodeClickFunction = function(){		
	$('.nodeClick').click(function(){
		clearSelection();
		$(this).css({'background-color': '#2fb1d1', 'color': 'white'});
		selectedNode = $(this).attr('id');
		//show analyze button if there is more than 2 children
		if(map[$(this).attr('id')].children.length > 2){
			analyzeNodeBtn.removeClass("inactive-button");
			analyzeNodeBtn.addClass("context-menu-button-relate");
		}else{
			analyzeNodeBtn.addClass("inactive-button");
			analyzeNodeBtn.removeClass("context-menu-button-relate");
		}
		
		//show relate if this is not the root, and there is more than 2 siblings
		console.log($(this).attr('id'));
		if($(this).attr('id') != 'node_root'){ 
			if(map[map[selectedNode].parent].children.length > 2){
				relateNodeBtn.removeClass("inactive-button");
				relateNodeBtn.addClass("context-menu-button-relate");
			}else{
				relateNodeBtn.addClass("inactive-button");
				relateNodeBtn.removeClass("context-menu-button-relate");
			}
		}else{
			relateNodeBtn.addClass("inactive-button");
			relateNodeBtn.removeClass("context-menu-button-relate");
		...
		//show node menu if relation mode is disable
		if(raltionMode == false){
			showNodeMenu(event);
		}
...
var treeConfig = {
		...
		callback: {
			onTreeLoaded : defaultNodeClickFunction
		},
		"connectors":{
			"style":{
				"stroke": "#ffffff"
    ...
    nodeStructure: {
		HTMLid: "node_root",
        text: { name: "Root", desc:"", weight: 100},
		parent: "",
        children: [
...
//target is parent node refrence, not ID
/*
	Adds a node under a target parent node
	target: parent node refrence
	name: string
*/
function addNode(target, name, desc){
	...
	target.children.push({
		HTMLid: newId,
		text: {name: name, desc: desc, weight: 100},
...
/*
	...
	input:
		...
		- name: string
*/
function editNodeName(id, name, desc){
	map[id].text.name = name;
	map[id].text.desc = desc;
}
