...
var editingSelectionFunction = function(){
	...
	$('.nodeClick').contextmenu(function(event){
		clearSelection();
		$(this).css({'background-color': '#2fb1d1', 'color': 'white'});
		selectedNode = $(this).attr('id');
		//rcMenu location
		rcMenu.css({left: event.pageX, top: event.pageY, zIndex: 250, display: 'inline-table'});
		rcMenu.show();
...
var rcMenu = $('#rcMenu');
$('#editTreeChart').click(function(){
	rcMenu.hide();
});
$('#editTreeChart').contextmenu(function(){
	return false;
});
